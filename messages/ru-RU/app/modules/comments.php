<?php

return [
    'Comments' => "Комментарии",
    'Tree comments system' => "Древовидная система комментариев",

    'No comments available' => "Нет доступных для отображения комментариев",
    'An error occurred while retrieving the comments list' => "Произошла ошибка при получении списка комментариев",

    'First page' => 'Первая страница',
    'Last page' => 'Последняя страница',
    '&larr; Prev page' => '&larr; Предыдущая страница',
    'Next page &rarr;' => 'Следующая страница &rarr;',
    '&larr; Back to list' => '&larr; Назад к списку',

    'Comments list' => "Список комментариев",
    'Context `{context}`, target `{target}`' => "Контекст `{context}`, цель `{target}`",
    'OK! {count, number} {count, plural, one{comment} few{comments} other{comments}} successfully {count, plural, one{updated} few{updated} other{updated}}.' => "OK! {count, number} {count, plural, one{комментарий} few{комментария} other{комментариев}} успешно {count, plural, one{обновлен} few{обновлено} other{обновлены}}.",
    'An error occurred while updating a comment(s).' => "Произошла ошибка при обновлении комментария(ев).",
    'OK! {count, number} {count, plural, one{comment} few{items} other{comments}} successfully {count, plural, one{deleted} few{deleted} other{deleted}}.' => "OK! {count, number} {count, plural, one{комментарий} few{комментария} other{комментариев}} успешно {count, plural, one{удален} few{удалено} other{удалены}}.",
    'An error occurred while deleting a comment(s).' => "Произошла ошибка при удалении комментария(ев).",

    'ID' => "ИД",
    'Parent ID' => "ИД родителя",
    'Context' => "Контекст",
    'Target' => "Цель",
    'Name' => "Имя",
    'E-mail' => "Эл. почта",
    'Comment' => "Комментарий",
    'User' => "Пользователь",
    'User ID' => "ИД пользователя",
    'Status' => "Статус",
    'Session' => "Сессия",
    'Created' => "Создано",
    'Updated' => "Обновлено",

    'OK! Your comment successfully sended but awaiting to moderation.' => "OK! Ваш комментарий успешно отправлен, но ожидает модерации.",
    'OK! Your comment successfully published.' => "OK! Ваш комментарий успешно опубликован.",
    'An error occurred while add your comment.' => "Произошла ошибка при добавлении вашего комментария.",
    'The requested comment does not exist.' => "Запрашиваемый комментарий не существует.",
    'OK! Your comment successfully updated.' => "OK! Ваш комментарий успешно обновлен.",
    'An error occurred while updating your comment.' => "Произошла ошибка при обновлении вашего комментария.",
    'OK! Your comment successfully deleted.' => "OK! Ваш комментарий успешно удален.",
    'An error occurred while deleting a your comment.' => "Произошла ошибка при удалении вашего комментария.",
    'OK! Your abuse to comment successfully sended.' => "OK! Ваша жалоба на комментарий успешно отправлена.",
    'An error occurred while sending your abuse to comment.' => "Произошла ошибка при отправке вашей жалобы на комментарий.",

    'Abuse to comment' => "Жалоба на комментарий",
    'User {username}' => "Пользователь {username}",
    'user {username}' => "пользователя {username}",
    'Guest' => "Гость",
    'guest {name}' => "гостя {name}",
    'Hi! {sender} has submitted a complaint about comment `{comment}` by {author}. You can moderate the comment here: {url}' => "Приветствуем! {sender} оставил жалобу на комментарий `{comment}` от {author}. Вы можете модерировать данный комментарий по адресу: {url}",

    'Unfortunately, you can no longer edit this comment.' => "К сожалению, Вы уже не можете редактировать этот комментарий.",
    'Unfortunately, you can no longer delete this comment.' => "К сожалению, Вы уже не можете удалить этот комментарий.",

    'Leave a comment' => "Оставить комментарий",
    'Submit' => "Отправить",
    'Comments ({0})' => "Комментарии ({0})",
    'Edit' => "Изменить",
    'Abuse' => "Жалоба",
    'Reply' => "Ответить",
    'Delete' => "Удалить",
    'Manage' => "Управление",

    'Author' => "Автор",
    'User: {username}' => "Пользователь: {username}",
    'Guest' => "Гость",
    'Guest: {name}' => "Гость: {name}",

    'View comment' => "Просмотр комментария",
    'Update comment' => "Изменить комментарий",
    'Close' => "Закрыть",
    'Save' => "Сохранить",

    'Comment has been deleted.' => "Комментарий был удален.",

    'All contexts' => "Все контексты",
    'All targets' => "Все цели",
    'All ranges' => "Все диапазоны",
    'Less than 1K comments' => "Менее 1К комментариев",
    'Over 1K comments' => "Более 1К комментариев",
    'Over 10K comments' => "Более 10К комментариев",
    'Over 100K comments' => "Более 100К комментариев",
    'More than 1M comments' => "Более 1М комментариев",
    'More than 10M comments' => "Более 10М комментариев",
    'All statuses' => "Все статусы",
    'Rejected' => "Отклонён",
    'Deleted' => "Удалён",
    'Awaiting moderation' => "Ожидает модерации",
    'On moderation' => "На модерации",
    'Published' => "Опубликован",

    'Are you sure you want to permanently delete the comment? All replies to this comment will also be deleted.' => "Вы уверены, что желаете безвозвратно удалить комментарий? Все ответы на данный комментарий также будут удалены.",

    'Select action' => "Выберите действие",
    'Change status to: {name}' => "Изменить статус на: {name}",
    'Delete selected' => "Удалить выбранное",
];

?>